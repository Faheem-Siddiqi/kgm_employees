...
- Bulk upload saves time by rejecting unsupported file types, unmatched names, locked/already-saved DB documents, different existing storage files with the same saved name, and duplicate matches before compression work starts. If the source file is already the exact file in the configured employee storage folder and the DB field is empty, the app saves the missing DB path without creating a duplicate copy. Compressed temp files are cleaned after home bulk copies and when pending panel uploads are replaced or cleared.
